...
async def initiate_connection(
    ...
    composio_client functions are now async.
    """
    result = await composio_client.initiate_connection(app)
    ...
    if result.redirect_url is None:
        row = await _activate(db, row, result.account_id, app)

    return _row_to_connection(row), result.redirect_url


async def handle_callback(db: Client, connection_id: str) -> Connection:
    ...
    status = await composio_client.get_connection_status(account_id)

    if status == ConnectionStatus.CONNECTED:
        row = await _activate(db, row, account_id, row["app"])
    else:
...
async def _activate(db: Client, row: dict, account_id: str, app: str) -> dict:
    mcp_url = composio_client.get_mcp_url(app, account_id)
    tools = await composio_client.get_available_tools(app, account_id)
    updated = {
        **row,
